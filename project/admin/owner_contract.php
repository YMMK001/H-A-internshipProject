                                        ...
                                        <?php while($row = $result->fetch_assoc()): ?>
                                            <tr class="hover:bg-gray-50 transition-colors odd:bg-stone-50/50">
                                                ...
                                                <td class="p-3 pl-4 font-bold text-gray-500 border-r border-gray-200">
                                                    #<?php echo htmlspecialchars($row['contract_id'] ?? ''); ?>
                                                </td>
                                                ...
                                                <td class="p-3 border-r border-gray-200 font-medium text-gray-700">
                                                    <div class="flex items-center gap-1.5">
                                                        ...
                                                        <span class="font-mono border border-gray-300 bg-white px-1.5 py-0.5"><?php echo htmlspecialchars($row['start_date'] ?? ''); ?></span>
                                                    </div>
                                                    <div class="flex items-center gap-1.5 mt-1.5">
                                                        <span class="text-rose-800 font-bold text-[10px] uppercase tracking-wider w-10">End:</span>
                                                        <span class="font-mono border border-gray-300 bg-white px-1.5 py-0.5"><?php echo htmlspecialchars($row['end_date'] ?? ''); ?></span>
                                                    </div>
                                                </td>

                                                <td class="p-3 font-bold text-gray-900 text-right tracking-tight">
                                                    <?php echo number_format($row['total_deposit_amount'] ?? 0); ?> <span class="text-[10px] font-normal text-gray-500 ml-0.5">MMK</span>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="5" class="p-8 text-center text-gray-400 italic bg-gray-50">
                                                တင်သွင်းထားသော สာချုပ်လျှောက်လွှာ မရှိသေးပါ။
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
